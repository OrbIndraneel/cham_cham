...
class SpatialGraphBuilder:
    ...
    def build_spatial_grid_graph(
        self, 
        center_lat: float, 
        center_lng: float, 
        rainfall_mm: float, 
        river_level_m: float = 0.0,
        slope_angle_deg: float = 38.5,
        soil_moisture_pct: float = 82.0,
        elevation_m: float = 2200.0,
        ...
        raw_features = [
            [rainfall_mm, river_level_m or 0.0, slope_angle_deg, soil_moisture_pct, elevation_m, vegetation_ndvi],          # Node 0 (Target)
            [rainfall_mm * 1.1, river_level_m * 1.2, slope_angle_deg + 3.5, min(100.0, soil_moisture_pct * 1.07), elevation_m + 400.0, max(0.1, vegetation_ndvi - 0.1)],      # Node 1 (Upstream High Slope)
            [rainfall_mm * 0.9, river_level_m * 0.9, max(5.0, slope_angle_deg - 13.5), soil_moisture_pct * 0.85, max(100.0, elevation_m - 300.0), min(0.9, vegetation_ndvi + 0.15)],      # Node 2 (Downstream Valley)
            [rainfall_mm * 1.05, river_level_m * 1.0, slope_angle_deg - 3.5, soil_moisture_pct * 0.98, elevation_m + 200.0, max(0.1, vegetation_ndvi - 0.05)],     # Node 3 (Adjacent Ridge)
            [rainfall_mm * 0.95, river_level_m * 0.8, max(5.0, slope_angle_deg - 20.5), soil_moisture_pct * 0.79, max(100.0, elevation_m - 450.0), min(0.9, vegetation_ndvi + 0.25)]      # Node 4 (Relief Station Base)
        ]
